...
rownames(default_payoff) <- c("A1", "A2", "A3")
colnames(default_payoff) <- c("畅销", "一般", "滞销")

default_probs <- c(0.3, 0.5, 0.2)
      ...
      tabsetPanel(
        tabPanel(
          ...
          div(
            ...
            tags$hr(),
            h4("默认案例"),
            tags$p("某企业生产新产品，有三种备选方案 A1、A2、A3，面临畅销、一般、滞销三种市场状态。"),
            tags$p("默认收益矩阵（单位：万元）已预置，可直接点击“计算决策结果”查看各准则推荐方案。")
          )
        ),
